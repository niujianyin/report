/* [2,717,242] published at 2014-12-19 16:49:27 */
...
var feedTemplate0 = (function(){
  ...
  html.push('<div class="feed-author" id=""><%=news.media_name%></div>');
...
var NotificationGuarder = (function(){
  ...
  NotificationGuarderClass.prototype = {
    ...
    _sendRequest: function(){
      var that = this;
      IO.jsonp({
        url: 'http://v.feed.mix.sina.com.cn/api/roll/last_news_count',
...
var FeedCard = (function(){
  ...
  var TIP_HTML = '<div class="feed-card-guessTips" id="feedCardGuessTips"><div class="feed-card-guessTipsC"><p class="feed-card-guessTipsT">\u8FD9\u91CC\u662F\u63A8\u8350\u5F15\u64CE\u4E3A\u4F60\u63A8\u8350\u7684\u5174\u8DA3\u5185\u5BB9\uFF0C</p><p class="feed-card-guessTipsT">\u8D8A\u591A\u4F7F\u7528\u8D8A\u61C2\u4F60~</p><span class="feed-card-guessTipsBtn" onclick="__SinaFeedCard__.hideGuessLike();">\u77E5\u9053\u4E86</span></div><span class="feed-card-guessTipsA"></span></div>';
  ...
  FeedCard.prototype = {
    ...
    _loadNews: function(page, step, renderType){
      ...
      switch(that._iotype){
        ...
        // 猜你喜欢
        case 2:
          url = that.guessLikeURL + '?' + that.creQuery;
          data = {
            num: that.pageSize,
            lid: that.lid,
            versionNumber: that.versionNumber
    ...
    _dealNewsItem: function(news){
      ...
      // 如果没有url，过滤掉
      // 但是，社交推荐没有标题，不过滤
      if(!news.url && news.templateid != 2 && news.templateid != 1001 && news.templateid != 1002){
        return false;
      }
      ...
      // 如果是1001或者1002
      if(news.templateid == 1001 || news.templateid == 1002) {
        if(news.sub && news.sub.length){
          for(var j = 0; j < news.sub.length; j ++){
            that._dealNewsItem(news.sub[j]);
          }
      ...
      if(news.templateid === 107){
        if(SINA_IMG_REG.test(news.img.u)){
          news.img.u = "http://s.img.mix.sina.com.cn/auto/resize?img=" + news.img.u + "&size=" +
            300 + '_' + 200;
        }
      }
    ...
    _renderNews: function(newsList, renderType){
      ...
      if(newsList.length){
        ...
        if(!storage.getItem('feed_rendered_guessLikeTip')) {
          if(that.step == 1 && that.page == 1 && that.lid == -101) {
            html = TIP_HTML + html;
          }
        }
  ...
  TabClass.prototype = {
    ...
    renderTabs: function(tabRenderLength){
      ...
      for(var i = 0; i < tabRenderLength; i ++){
        ...
        } else if(tabInfo.lid == -101) {
          html.push('<span class="feed-card-tab-tabi" data-lid="' + tabInfo.lid + '" style="width:' + tabInfo.css_width + 'px;" suda-uatrack="key=index_feed&value=cate_float:' + tabInfo.lid + '"><i class="feed-card-guessLikeIcon"></i>' + tabInfo.txt + '</span>');
        } else {
...
;(function(){
  ...
  win.__SinaFeedCard__ = {
    ...
    hideGuessLike: function() {
      storage.setItem('feed_rendered_guessLikeTip', '1');
      try {
        dom.addClass(dom.$('feedCardGuessTips'), 'feed-card-guessTips-hide');
      }catch(e){}
    }
  };
...
//  图片和容器
 dom_feedCard1.find(" .feed-card-img img").css({
   width:"260px",
   height:"176px"
 }).each(function(){
   var cardSrc=$(this).attr("src");
   var cardNewSrc=cardSrc.split("130_87").join("300_200");
   $(this).attr("src",cardNewSrc);
 })
 dom_feedCard1.find(".feed-card-img").css({
   width:"260px",
   height:"176px",
   marginTop:0,
   marginRight:0
 });
 dom_feedCard1.find(".feed-card-c1").css("marginTop","0");
  dom_feedCard1.find(".feed-card-txt").css({
    width:"480px",
    position:"absolute",
    left:"320px",
    top:"94px"
  ...
  // 时间
  dom_feedCard1.find(".feed-card-time").css({
    // width:"480px",
    position:"absolute",
    left:"320px",
    top:"60px"
  ...
  // 来源
   dom_feedCard1.find(".feed-author").each(function(){
     var timeWidth=$(this).parents(".feed-card-item").find(".feed-card-time").width();
     $(this).css({
     position:"absolute",
     left:320+timeWidth+15+"px",
    top:"60px",
    fontSize:"14px",
    color:"#555"
   });
  ...
  //  tag
   dom_feedCard1.find(".feed-card-tags").css({
     position:"absolute",
    left:"300px",
    // top:"60px",
   });
